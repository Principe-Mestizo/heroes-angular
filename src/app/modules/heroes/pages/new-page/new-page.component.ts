...
import { IApiResponseHeroes, Publisher } from '../../../../models/heroes.interface';
...
export default class NewPageComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this._activatedRoutes.params.pipe(switchMap(({ id }) => this._heroeService.getHeroById(id))).subscribe((hero) => {
			if (!hero) {
		...
		this._modalService.confirm({
			...
			nzOnOk: () => {
				this._heroeService.deleteHeroById(this.currentHero.id).subscribe({
					...
					error: () => {
						this._toastr.warning('Ocurrió un error al eliminar el héroe.', 'Error');
					}
				});
			},
			nzCancelText: 'No',
